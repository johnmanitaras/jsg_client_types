...
### REST API Features
- Automatic tenant header management (`X-DB-Name`)
- API key handling
...
- Automatic tenant header management (`X-Hasura-Tenant-Id`)
...
JetSetGo uses a **multi-tenant Hasura architecture**. Key points:

1. **Table names use a static prefix**: `jsg_reference_schema_`
2. **Tenant isolation is via header**: `X-Hasura-Tenant-Id`
3. **Do NOT use dynamic tenant prefixes** in table names (e.g., `tta_`, `${tenantName}_`)

For complete migration guide, see: `C:\Users\mail\Downloads\FRONTEND_HASURA_MIGRATION_GUIDE.md`
...
function YourComponent() {
  ...
  const fetchInventoryData = async () => {
    try {
      // Use jsg_reference_schema_ prefix for all tables
      // Tenant is automatically included via X-Hasura-Tenant-Id header
...
const getUnitById = async (unitId: string) => {
  const response = await query(`
    query GetUnitById($id: String!) {
      jsg_reference_schema_inventory_holding_units(where: {id: {_eq: $id}}) {
        id
        name
        business_id
      }
...
const createUnit = async (unitData: any) => {
  const response = await mutate(`
    mutation CreateUnit($data: jsg_reference_schema_inventory_holding_units_insert_input!) {
      insert_jsg_reference_schema_inventory_holding_units_one(object: $data) {
        id
        name
        business_id
      }
...
const getUnitsWithName = async (variables: any) => {
  const response = await query(`
    query GetUnitsWithName($limit: Int) {
      jsg_reference_schema_inventory_holding_units(limit: $limit) {
        id
        name
      }
    }
...
## GraphQL Table Naming Convention

| Type | Pattern | Example |
|------|---------|---------|
| Query | `jsg_reference_schema_tablename` | `jsg_reference_schema_bookings` |
| Mutation (insert) | `insert_jsg_reference_schema_tablename_one` | `insert_jsg_reference_schema_clients_one` |
| Mutation (update) | `update_jsg_reference_schema_tablename_by_pk` | `update_jsg_reference_schema_bookings_by_pk` |
| Mutation (delete) | `delete_jsg_reference_schema_tablename_by_pk` | `delete_jsg_reference_schema_clients_by_pk` |
| Aggregate | `jsg_reference_schema_tablename_aggregate` | `jsg_reference_schema_bookings_aggregate` |
...
function CreateInventoryUnit() {
  ...
  const handleSubmit = async (unitData) => {
    try {
      const response = await mutate(`
        mutation CreateInventoryUnit($data: jsg_reference_schema_inventory_holding_units_insert_input!) {
          insert_jsg_reference_schema_inventory_holding_units_one(object: $data) {
            id
            name
            business_id
...
3. **DO NOT** use dynamic tenant prefixes in table names (e.g., `${tenantName}_tablename`).
4. **DO NOT** hardcode tenant-specific prefixes (e.g., `tta_tablename`).
5. **ALWAYS** use the `jsg_reference_schema_` prefix for all table names.
6. **ALWAYS** use proper GraphQL syntax in your queries and mutations.
7. **DO NOT** store sensitive credentials in components.
...
For API configuration, set these environment variables:
...
**Note**: `VITE_DEFAULT_TENANT` is used for the `X-Hasura-Tenant-Id` header in standalone mode. It is NOT used for table name prefixing - all tables use the static `jsg_reference_schema_` prefix.
...
7. **Don't** use dynamic tenant prefixes in GraphQL table names
8. **Don't** hardcode tenant-specific prefixes like `tta_`

## Need Help?

For complete GraphQL migration documentation, see:
- `C:\Users\mail\Downloads\FRONTEND_HASURA_MIGRATION_GUIDE.md`
- `C:\Users\mail\Desktop\jetsetgo\app\HASURA_MIGRATION_PROMPT.md`

For API integration patterns, see:
- `C:\Users\mail\Desktop\jetsetgo\app\jsg_wrapper\docs\child-app-integration-standards.md` (Section 6)
